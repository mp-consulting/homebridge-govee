...
/**
 * Temperature/Humidity sensor with thermostat display.
 * Shows as a Thermostat in HomeKit with read-only temperature.
 * Supports temperature, humidity, and battery level monitoring.
 */
export class SensorThermoSwitchDevice extends GoveeDeviceBase {
  private thermoService!: Service;
  ...
  get service(): Service {
    return this.thermoService;
  }

  init(): void {
    // Remove temperature sensor service if it exists (use Thermostat instead)
    ...
    this.cacheTemp = this.thermoService.getCharacteristic(this.hapChar.CurrentTemperature).value as number || 20;
    ...
    // Set heating/cooling state to OFF (0) - sensor only
